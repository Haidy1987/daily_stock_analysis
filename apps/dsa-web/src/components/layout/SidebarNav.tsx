...
const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'admin-users', labelKey: 'layout.nav.adminUsers', to: '/admin/users', icon: Users, adminOnly: true },
...
export const SidebarNav: React.FC<SidebarNavProps> = ({ collapsed = false, onNavigate, variant = 'default' }) => {
  ...
  const navItems = NAV_ITEMS.filter((item) => {
    if (item.key === 'screening' && !showAlphaSiftNav) {
      return false;
    }
    if (item.key === 'account' && !authEnabled) {
      return false;
    }
    if (item.adminOnly && (!authEnabled || !isAdmin)) {
      return false;
    }
    return true;
  });
