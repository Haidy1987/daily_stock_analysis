...
2. **Docker deployment / WebUI Settings**:
   ...
   - WebUI saves `STOCK_LIST`, `SCHEDULE_ENABLED`, `SCHEDULE_TIME`, `SCHEDULE_TIMES`, `SCHEDULE_RUN_IMMEDIATELY`, and `RUN_IMMEDIATELY` into the active runtime config file; Docker Compose uses the data-volume `/app/data/runtime.env` by default
   - Saving from WebUI triggers a config reload for the current process, and runtime reads continue from the latest persisted runtime config file; for example, scheduled runs keep hot-reading the saved `STOCK_LIST`
   ...
   - For Docker Compose, initialize `data/runtime.env` with `cp .env data/runtime.env` on the first deployment; the project sets `ENV_FILE=/app/data/runtime.env` so WebUI saves go to the data volume, and later releases must not overwrite `data/`
   - Do not bind-mount the host `.env` as a single file over `/app/.env` or keep the host `.env` as the container runtime `env_file`, otherwise it can override WebUI-saved AI settings
