...
from src.schemas.decision_action import DecisionAction, build_action_fields, localize_action_label, normalize_decision_action
...
from src.storage import AnalysisHistory, DatabaseManager, DecisionSignalRecord, to_utc_naive_datetime, utc_naive_now
...
INTRADAY_PHASES = frozenset({MarketPhase.PREMARKET.value, MarketPhase.INTRADAY.value, MarketPhase.LUNCH_BREAK.value, MarketPhase.CLOSING_AUCTION.value})
DEFAULT_INTRADAY_TTL_HOURS = {'cn': 4.0, 'hk': 5.5, 'us': 6.5}
...
class DecisionSignalService:
    ...
    def __init__(self, repo: Optional[DecisionSignalRepository]=None, portfolio_repo: Optional[PortfolioRepository]=None, db_manager: Optional[DatabaseManager]=None):
    ...
    def create_signal(self, payload: Dict[str, Any], *, user_id: Optional[int] = None) -> Dict[str, Any]:
        user_id = resolve_service_user_id(user_id)
        (fields, lifecycle) = self._normalize_payload(payload, user_id=user_id)
        result = self.repo.create_if_absent(fields, allow_relaxed_horizon_fill=lifecycle['horizon_defaulted'])
        if result.row.status == 'active':
            self._invalidate_opposing_active_signals(result.row, reference_at=result.invalidation_reference_at)
        return {'item': self._serialize(result.row), 'created': result.created}

    def get_signal(self, signal_id: int, *, user_id: Optional[int] = None) -> Dict[str, Any]:
        user_id = resolve_service_user_id(user_id)
    ...
    def list_signals(self, *, user_id: Optional[int] = None, stock_code: Optional[str]=None, market: Optional[str]=None, action: Optional[str]=None, market_phase: Optional[str]=None, source_type: Optional[str]=None, source_report_id: Optional[Any]=None, trace_id: Optional[str]=None, trigger_source: Optional[str]=None, status: Optional[str]=None, created_from: Optional[Any]=None, created_to: Optional[Any]=None, expires_from: Optional[Any]=None, expires_to: Optional[Any]=None, holding_only: bool=False, account_id: Optional[int]=None, stock_identities: Optional[List[Tuple[str, str]]]=None, page: int=1, page_size: int=20) -> Dict[str, Any]:
        user_id = resolve_service_user_id(user_id)
        ...
        if stock_identities is not None:
            ...
            if market_norm:
                held_identities = {identity for identity in held_identities if identity[0] == market_norm}
            if stock_codes:
                requested_codes = set(stock_codes)
                held_identities = {identity for identity in held_identities if identity[1] in requested_codes}
            stock_identity_filters = sorted(held_identities)
            stock_codes = None
            if not stock_identity_filters:
                return {'items': [], 'total': 0, 'page': safe_page, 'page_size': safe_page_size}
        (rows, total) = self.repo.list(user_id=user_id, stock_codes=stock_codes, stock_identities=stock_identity_filters, market=market_norm, action=action_norm, market_phase=market_phase_norm, source_type=source_type_norm, source_report_id=source_report_id_norm, trace_id=trace_id_norm, trigger_source=trigger_source_norm, status=status_norm, created_from=created_from_dt, created_to=created_to_dt, expires_from=expires_from_dt, expires_to=expires_to_dt, page=safe_page, page_size=safe_page_size)
        if total == 0 and self._should_backfill_history_bound_analysis_signal(stock_code=stock_code, market=market_norm, action=action_norm, market_phase=market_phase_norm, source_type=source_type_norm, source_report_id=source_report_id_norm, trace_id=trace_id_norm, trigger_source=trigger_source_norm, status=status_norm, created_from=created_from_dt, created_to=created_to_dt, expires_from=expires_from_dt, expires_to=expires_to_dt, stock_identities=stock_identity_filters, holding_only=holding_only):
            self._backfill_analysis_signal_from_history(source_report_id_norm, user_id=user_id)
            (rows, total) = self.repo.list(stock_codes=stock_codes, stock_identities=stock_identity_filters, market=market_norm, action=action_norm, market_phase=market_phase_norm, source_type=source_type_norm, source_report_id=source_report_id_norm, trace_id=trace_id_norm, trigger_source=trigger_source_norm, status=status_norm, created_from=created_from_dt, created_to=created_to_dt, expires_from=expires_from_dt, expires_to=expires_to_dt, page=safe_page, page_size=safe_page_size)
        return {'items': [self._serialize(row) for row in rows], 'total': total, 'page': safe_page, 'page_size': safe_page_size}

    def get_latest_active(self, *, user_id: Optional[int] = None, stock_code: str, market: Optional[str]=None, limit: int=1) -> Dict[str, Any]:
        user_id = resolve_service_user_id(user_id)
        market_norm = self._normalize_optional_market(market)
        rows = self.repo.get_latest_active(user_id=user_id, stock_codes=self._stock_filter_codes(stock_code, market=market_norm) or [self._normalize_stock_code(stock_code)], market=market_norm, limit=limit)
        return {'items': [self._serialize(row) for row in rows], 'total': len(rows), 'page': 1, 'page_size': max(1, min(int(limit), 100))}

    def update_status(self, signal_id: int, *, user_id: Optional[int] = None, status: str, metadata: Optional[Any]=None, replace_metadata: bool=False) -> Dict[str, Any]:
        user_id = resolve_service_user_id(user_id)
        ...
        if existing is None:
            raise DecisionSignalNotFoundError(f'Decision signal not found: {signal_id}')
        if status_norm == 'active' and (existing.status in TERMINAL_STATUSES or self._is_expired(existing.expires_at)):
            raise ValueError('terminal decision signal cannot be reactivated through status update')
        row = self.repo.update_status(signal_id, user_id=user_id, status=status_norm, metadata_json=metadata_json, replace_metadata=replace_metadata)
        if row is None:
            raise DecisionSignalNotFoundError(f'Decision signal not found: {signal_id}')
        return self._serialize(row)

    @staticmethod
    def _should_backfill_history_bound_analysis_signal(*, stock_code: Optional[Any], market: Optional[str], action: Optional[str], market_phase: Optional[str], source_type: Optional[str], source_report_id: Optional[int], trace_id: Optional[str], trigger_source: Optional[str], status: Optional[str], created_from: Optional[datetime], created_to: Optional[datetime], expires_from: Optional[datetime], expires_to: Optional[datetime], stock_identities: Optional[List[Tuple[str, str]]], holding_only: bool) -> bool:
        """Only lazy-backfill for the exact report section query used by Web."""
        if source_type != 'analysis' or source_report_id is None:
            return False
        return not any((value not in (None, '', False) for value in (stock_code, market, action, market_phase, trace_id, trigger_source, status, created_from, created_to, expires_from, expires_to, stock_identities, holding_only)))

    def _backfill_analysis_signal_from_history(self, source_report_id: int, *, user_id: Optional[int] = None) -> None:
        ...
        try:
            ...
            (history_action, history_action_label) = self._history_action_fields(raw=raw, record=record)
            ...
            result = AnalysisResult(code=getattr(record, 'code', '') or '', name=getattr(record, 'name', None) or raw.get('name') or '', sentiment_score=self._history_int(raw.get('sentiment_score'), getattr(record, 'sentiment_score', None), default=50), trend_prediction=raw.get('trend_prediction') or getattr(record, 'trend_prediction', None) or '', operation_advice=raw.get('operation_advice') or getattr(record, 'operation_advice', None) or '', decision_type=raw.get('decision_type') or '', confidence_level=raw.get('confidence_level') or '中', report_language=normalize_report_language(raw.get('report_language')), action=history_action, action_label=history_action_label, dashboard=raw.get('dashboard') if isinstance(raw.get('dashboard'), dict) else None, analysis_summary=raw.get('analysis_summary') or getattr(record, 'analysis_summary', None) or '', key_points=raw.get('key_points') or '', risk_warning=raw.get('risk_warning') or '', buy_reason=raw.get('buy_reason') or '', raw_response=raw.get('raw_response'), search_performed=bool(raw.get('search_performed', False)), data_sources=raw.get('data_sources') or '', success=bool(raw.get('success', True)), error_message=raw.get('error_message'), current_price=self._history_float(raw.get('current_price')), change_pct=self._history_float(raw.get('change_pct')), model_used=raw.get('model_used'), query_id=getattr(record, 'query_id', None))
            payload = build_decision_signal_payload_from_report(result, user_id=user_id, context_snapshot=context_snapshot, source_report_id=source_report_id, trace_id=str(getattr(record, 'query_id', '') or source_report_id), query_source='history', report_type=str(getattr(record, 'report_type', '') or 'simple'), profile_source='backfill_defaulted')
            if payload is None:
                return
            self._apply_history_backfill_lifecycle(payload, created_at=getattr(record, 'created_at', None))
            ...
            logger.warning('Decision signal lazy backfill failed: source_report_id=%s error=%s', source_report_id, exc, exc_info=True)
    ...
    @staticmethod
    def _history_action_fields(*, raw: Dict[str, Any], record: AnalysisHistory) -> tuple[Optional[str], Optional[str]]:
        ...
        score = DecisionSignalService._history_int(raw.get('sentiment_score'), getattr(record, 'sentiment_score', None), default=None)
        raw_action_value = normalize_decision_action(normalized_action) or normalize_decision_action(normalized_operation_advice)
        guardrail_reason = DecisionSignalService._history_guardrail_reason(raw=raw, operation_advice=normalized_operation_advice, score=score, raw_action=raw_action_value)
        action_fields = build_action_fields(operation_advice=normalized_operation_advice, explicit_action=normalized_action, report_type=getattr(record, 'report_type', ''), report_language=raw.get('report_language'), sentiment_score=score, guardrail_reason=guardrail_reason, align_with_score=True)
        return (action_fields['action'], action_fields['action_label'])

    @staticmethod
    def _history_guardrail_reason(*, raw: Dict[str, Any], operation_advice: Optional[str], score: Optional[int], raw_action: Optional[str]) -> Optional[str]:
        dashboard = raw.get('dashboard') if isinstance(raw.get('dashboard'), dict) else {}
        calibration = dashboard.get('decision_score_calibration') if isinstance(dashboard.get('decision_score_calibration'), dict) else {}
        stability = dashboard.get('decision_stability') if isinstance(dashboard.get('decision_stability'), dict) else {}
        for candidate in (calibration.get('guardrail_reason'), stability.get('reason'), raw.get('guardrail_reason')):
        ...
        if score_action_conflicts_without_guardrail(score=score, action=raw_action):
            ...
            if action_for_score(score) == 'buy':
                candidates.extend([raw.get('analysis_summary'), raw.get('buy_reason'), raw.get('risk_warning')])
            hints = ('等待', '待', '需要确认', '缺少确认', '未确认', '回踩', '支撑', '压力', '风险', '资金', '突破', '不追', '不宜')
    ...
    def _apply_history_backfill_lifecycle(self, payload: Dict[str, Any], *, created_at: Optional[datetime]) -> None:
        ...
        horizon = payload.get('horizon') or self._default_horizon(action=str(payload.get('action') or ''), market_phase=payload.get('market_phase'))
        if horizon:
            payload['horizon'] = horizon
        expires_at = self._history_backfill_expires_at(created_at=history_created_at, horizon=horizon, market=str(payload.get('market') or ''), metadata=payload.get('metadata'))
    ...
    def _invalidate_history_backfill_if_superseded(self, signal_id: int, *, user_id: Optional[int] = None) -> None:
        user_id = resolve_service_user_id(user_id)
        row = self.repo.get(signal_id, user_id=user_id)
        ...
        if not opposing_actions:
            return
        newer_rows = self.repo.list_active_by_stock_actions(user_id=user_id, market=row.market, stock_code=row.stock_code, actions=sorted(opposing_actions), exclude_signal_id=row.id)
        for newer_row in newer_rows:
            ...
            updated = self.repo.update_status(row.id, user_id=user_id, status='invalidated', metadata_json=metadata_json, replace_metadata=True)
            if updated is None:
                logger.warning('Decision signal disappeared before stale backfill invalidation: signal_id=%s invalidated_by=%s', row.id, newer_row.id)
            return

    @classmethod
    def _history_backfill_expires_at(cls, *, created_at: datetime, horizon: Optional[str], market: str, metadata: Any) -> Optional[datetime]:
        base = to_utc_naive_datetime(created_at)
        return cls._expires_at_from_base(horizon=horizon, market=market, metadata=metadata, base=base)
    ...
    def _normalize_payload(self, payload: Dict[str, Any], *, user_id: Optional[int] = None) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        user_id = resolve_service_user_id(user_id)
        ...
        if expires_at is None and (not expires_explicit):
            expires_at = self._default_expires_at(horizon=horizon, market=market, metadata=payload.get('metadata'))
        created_at = self._parse_datetime(payload.get('_created_at_override'))
        fields: Dict[str, Any] = {'user_id': user_id, 'stock_code': stock_code, 'stock_name': self._optional_public_text(payload.get('stock_name'), 'stock_name', max_length=64), 'market': market, 'source_type': self._normalize_enum(payload.get('source_type'), SOURCE_TYPES, 'source_type'), 'source_agent': self._optional_public_text(payload.get('source_agent'), 'source_agent', max_length=64), 'source_report_id': self._optional_int(payload.get('source_report_id'), 'source_report_id'), 'trace_id': self._optional_identity_text(payload.get('trace_id'), 'trace_id', max_length=64), 'market_phase': market_phase, 'trigger_source': self._normalize_trigger_source(payload.get('trigger_source')), 'action': action, 'action_label': action_label, 'confidence': confidence, 'score': score, 'horizon': horizon, 'entry_low': self._optional_price_float(payload.get('entry_low'), 'entry_low'), 'entry_high': self._optional_price_float(payload.get('entry_high'), 'entry_high'), 'stop_loss': self._optional_price_float(payload.get('stop_loss'), 'stop_loss'), 'target_price': self._optional_price_float(payload.get('target_price'), 'target_price'), 'invalidation': self._optional_signal_text(payload.get('invalidation')), 'watch_conditions': self._optional_signal_text(payload.get('watch_conditions')), 'reason': self._optional_signal_text(payload.get('reason')), 'risk_summary': self._optional_signal_text(payload.get('risk_summary')), 'catalyst_summary': self._optional_signal_text(payload.get('catalyst_summary')), 'evidence_json': self._json_dumps(payload.get('evidence')), 'data_quality_summary_json': self._json_dumps(payload.get('data_quality_summary')), 'status': self._normalize_optional_enum(payload.get('status'), SIGNAL_STATUSES, 'status') or 'active', 'expires_at': expires_at, 'metadata_json': self._json_dumps(payload.get('metadata'))}
        if created_at is not None:
            fields['created_at'] = created_at
        if fields['status'] == 'active' and self._is_expired(fields['expires_at']):
            fields['status'] = 'expired'
        self._validate_entry_range(fields)
        fields['plan_quality'] = self._normalize_plan_quality(payload.get('plan_quality'), fields=fields)
    ...
    @classmethod
    def _default_expires_at(cls, *, horizon: Optional[str], market: str, metadata: Any) -> Optional[datetime]:
        return cls._expires_at_from_base(horizon=horizon, market=market, metadata=metadata, base=utc_naive_now())

    @classmethod
    def _expires_at_from_base(cls, *, horizon: Optional[str], market: str, metadata: Any, base: datetime) -> Optional[datetime]:
        if horizon == 'intraday':
    ...
    def _invalidate_opposing_active_signals(self, row: DecisionSignalRecord, *, reference_at: Optional[datetime]) -> None:
        opposing_actions = self._opposing_actions(row.action)
        if not opposing_actions:
            return
        old_rows = self.repo.list_active_by_stock_actions(user_id=int(row.user_id or 0), market=row.market, stock_code=row.stock_code, actions=sorted(opposing_actions), exclude_signal_id=row.id)
        for old_row in old_rows:
            ...
            updated = self.repo.update_status(old_row.id, user_id=int(row.user_id or 0), status='invalidated', metadata_json=metadata_json, replace_metadata=True)
            if updated is None:
                logger.warning('Decision signal disappeared before invalidation: signal_id=%s invalidated_by=%s', old_row.id, row.id)

    @staticmethod
    def _is_prior_signal(candidate: DecisionSignalRecord, current: DecisionSignalRecord, *, reference_at: Optional[datetime]) -> bool:
    ...
    def _invalidation_metadata_json(self, row: DecisionSignalRecord, *, invalidated_by: DecisionSignalRecord) -> Optional[str]:
        metadata = self._metadata_for_invalidation(row)
        metadata.update({'invalidated_by_signal_id': invalidated_by.id, 'invalidated_reason': f'opposite_active_signal:{row.action}->{invalidated_by.action}', 'invalidated_at': utc_naive_now().isoformat(), 'previous_status': row.status})
    ...
    def _metadata_for_invalidation(row: DecisionSignalRecord) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning('Replacing invalid decision signal metadata during invalidation: id=%s error=%s', row.id, exc)
    ...
    def _cached_holding_identities(self, *, user_id: Optional[int] = None, account_id: Optional[int]) -> set[Tuple[str, str]]:
        user_id = resolve_service_user_id(user_id)
        identities = self.portfolio_repo.list_cached_position_identities(user_id=user_id, account_id=account_id)
    ...
    @classmethod
    def _stock_filter_codes(cls, stock_code: Optional[str], *, market: Optional[str]=None) -> Optional[List[str]]:
        if not stock_code:
            return None
    ...
    @classmethod
    def _normalize_optional_enum(cls, value: Any, allowed: frozenset[str], field_name: str) -> Optional[str]:
        if value in (None, ''):
            return None
    ...
    def _json_loads(value: Optional[str], *, signal_id: int, field_name: str) -> Any:
        ...
        try:
            ...
            logger.warning('Invalid decision signal JSON: id=%s field=%s error=%s', signal_id, field_name, exc)
            raise DecisionSignalStorageError(f'invalid persisted JSON for decision signal {signal_id} field {field_name}') from exc

    def _serialize(self, row: DecisionSignalRecord) -> Dict[str, Any]:
        return {'id': row.id, 'stock_code': row.stock_code, 'stock_name': row.stock_name, 'market': row.market, 'source_type': row.source_type, 'source_agent': row.source_agent, 'source_report_id': row.source_report_id, 'trace_id': row.trace_id, 'market_phase': row.market_phase, 'trigger_source': row.trigger_source, 'action': row.action, 'action_label': row.action_label, 'confidence': row.confidence, 'score': row.score, 'horizon': row.horizon, 'entry_low': row.entry_low, 'entry_high': row.entry_high, 'stop_loss': row.stop_loss, 'target_price': row.target_price, 'invalidation': row.invalidation, 'watch_conditions': row.watch_conditions, 'reason': row.reason, 'risk_summary': row.risk_summary, 'catalyst_summary': row.catalyst_summary, 'evidence': self._json_loads(row.evidence_json, signal_id=row.id, field_name='evidence_json'), 'data_quality_summary': self._json_loads(row.data_quality_summary_json, signal_id=row.id, field_name='data_quality_summary_json'), 'plan_quality': row.plan_quality, 'status': row.status, 'expires_at': row.expires_at.isoformat() if row.expires_at else None, 'created_at': row.created_at.isoformat() if row.created_at else None, 'updated_at': row.updated_at.isoformat() if row.updated_at else None, 'metadata': self._json_loads(row.metadata_json, signal_id=row.id, field_name='metadata_json')}
