...
from src.services.portfolio_service import PortfolioBusyError, PortfolioConflictError, PortfolioOversellError, PortfolioService
...
DEFAULT_PARSER_SPECS: Tuple[CsvParserSpec, ...] = (CsvParserSpec(broker='huatai', aliases=(), display_name='华泰', column_hints={'trade_date': ('成交日期', '成交时间', '发生日期', '日期'), 'symbol': ('证券代码', '股票代码', '代码'), 'side': ('买卖标志', '买卖方向', '操作'), 'quantity': ('成交数量', '数量', '成交股数'), 'price': ('成交均价', '成交价格', '价格', '成交价', '均价'), 'trade_uid': ('成交编号', '成交序号', '流水号')}), CsvParserSpec(broker='citic', aliases=('zhongxin',), display_name='中信', column_hints={'trade_date': ('发生日期', '成交日期', '日期'), 'symbol': ('证券代码', '股票代码', '代码'), 'side': ('买卖方向', '买卖标志', '业务名称'), 'quantity': ('成交数量', '数量', '成交股数'), 'price': ('成交价格', '成交均价', '价格', '成交价'), 'trade_uid': ('合同编号', '成交编号', '委托编号')}), CsvParserSpec(broker='cmb', aliases=('zhaoshang', 'cmbchina'), display_name='招商', column_hints={'trade_date': ('日期', '成交日期', '发生日期'), 'symbol': ('证券代码', '股票代码', '代码'), 'side': ('交易方向', '买卖方向', '买卖标志'), 'quantity': ('成交股数', '成交数量', '数量'), 'price': ('成交价', '成交价格', '成交均价', '均价'), 'trade_uid': ('流水号', '成交编号', '成交序号')}))
...
class PortfolioImportService:
    ...
    def __init__(self, *, portfolio_service: Optional[PortfolioService]=None, repo: Optional[PortfolioRepository]=None):
    ...
    def register_parser(self, spec: CsvParserSpec) -> None:
        ...
        self._parser_registry[broker] = CsvParserSpec(broker=broker, aliases=new_aliases, display_name=spec.display_name or broker, column_hints=dict(spec.column_hints or {}))
    ...
    def list_supported_brokers(self) -> List[Dict[str, Any]]:
        ...
        for broker in sorted(self._parser_registry.keys()):
            ...
            items.append({'broker': broker, 'aliases': aliases, 'display_name': self._parser_registry[broker].display_name})
        return items

    def parse_trade_csv(self, *, broker: str, content: bytes) -> Dict[str, Any]:
        ...
        return {'broker': broker_norm, 'record_count': len(records), 'skipped_count': skipped, 'error_count': len(errors), 'records': records, 'errors': errors[:20]}

    def commit_trade_records(self, *, user_id: Optional[int] = None, account_id: int, broker: str, records: List[Dict[str, Any]], dry_run: bool=False) -> Dict[str, Any]:
        ...
        for (i, record) in enumerate(records):
            try:
                ...
                self.portfolio_service.record_trade(user_id=user_id, account_id=account_id, symbol=str(record['symbol']), trade_date=trade_date_obj, side=str(record['side']), quantity=float(record['quantity']), price=float(record['price']), fee=float(record.get('fee', 0.0) or 0.0), tax=float(record.get('tax', 0.0) or 0.0), market=record.get('market'), currency=record.get('currency'), trade_uid=trade_uid, dedup_hash=dedup_hash_to_use, note=(record.get('note') or '').strip() or f'csv_import:{broker_norm}')
                inserted_count += 1
                ...
                failed_count += 1
                errors.append(f'idx={i}: {exc}')
        return {'account_id': account_id, 'record_count': len(records), 'inserted_count': inserted_count, 'duplicate_count': duplicate_count, 'failed_count': failed_count, 'dry_run': bool(dry_run), 'errors': errors[:20]}
    ...
    def _read_csv(content: bytes) -> pd.DataFrame:
        for encoding in ('utf-8-sig', 'gbk', 'gb18030'):
            try:
                return pd.read_csv(io.BytesIO(content), encoding=encoding, dtype=str, keep_default_na=False)
            except UnicodeDecodeError:
                continue
        return pd.read_csv(io.BytesIO(content), dtype=str, keep_default_na=False)

    def _normalize_trade_row(self, *, row: Any, parser_spec: CsvParserSpec) -> Optional[Dict[str, Any]]:
        broker_hints = parser_spec.column_hints
        trade_date_raw = self._pick(row, *(broker_hints.get('trade_date') or ()), '成交日期', '发生日期', '日期', '成交时间')
        trade_date_obj = self._parse_date(trade_date_raw)
        if trade_date_obj is None:
            return None
        symbol_raw = self._pick(row, *(broker_hints.get('symbol') or ()), '证券代码', '股票代码', '代码')
        symbol = canonical_stock_code(str(symbol_raw or '').strip())
        if not symbol:
            return None
        side_raw = self._pick(row, *(broker_hints.get('side') or ()), '买卖标志', '买卖方向', '交易方向', '业务名称', '操作')
        side = self._normalize_side(side_raw)
        if side is None:
            return None
        quantity = self._parse_float(self._pick(row, *(broker_hints.get('quantity') or ()), '成交数量', '数量', '成交股数'))
        price = self._parse_float(self._pick(row, *(broker_hints.get('price') or ()), '成交均价', '成交价格', '价格', '成交价', '均价'))
        ...
        for col in ('印花税', '税费', '其他税费'):
            ...
            if value is not None:
                tax += value
        trade_uid = self._pick(row, *(broker_hints.get('trade_uid') or ()), '成交编号', '成交序号', '合同编号', '委托编号', '流水号')
        currency = self._pick(row, '币种', '货币')
        return {'trade_date': trade_date_obj, 'symbol': symbol, 'side': side, 'quantity': float(quantity), 'price': float(price), 'fee': float(fee), 'tax': float(tax), 'trade_uid': (str(trade_uid).strip() if trade_uid is not None else None) or None, 'currency': (str(currency).strip().upper() if currency is not None else None) or None}
    ...
    def _build_dedup_hash(record: Dict[str, Any]) -> str:
        payload = '|'.join([str(record.get('trade_date') or ''), str(record.get('symbol') or ''), str(record.get('side') or ''), f"{float(record.get('quantity', 0.0)):.8f}", f"{float(record.get('price', 0.0)):.8f}", f"{float(record.get('fee', 0.0)):.8f}", f"{float(record.get('tax', 0.0)):.8f}", str(record.get('currency') or ''), str(record.get('_source_line_number') or record.get('source_line_number') or '')])
